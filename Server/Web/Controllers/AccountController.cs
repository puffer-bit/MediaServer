    ...
    public class AccountController(SignInManager<IdentityUser> signInManager, UserManager<IdentityUser> userManager) : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string? returnUrl)
        {
            if (model.IsGuest)
            {
                ...
                return View(returnUrl ?? "/Web/Views/Home/Index.cshtml");
            }
            else
            {
                ...
                if (result.Succeeded)
                {
                    return Redirect(returnUrl ?? "/Web/Views/Home/Index.cshtml");
                }
            }

            TempData["Error"] = "Login error: Wrong login data.";
            return View("/Web/Views/Account/Login.cshtml", model);
        }
