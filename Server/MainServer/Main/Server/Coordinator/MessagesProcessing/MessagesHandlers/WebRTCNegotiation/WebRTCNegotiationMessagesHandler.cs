...
namespace Server.MainServer.Main.Server.Coordinator.MessagesProcessing.MessagesHandlers.WebRTCNegotiation
{
    public class WebRTCNegotiationMessagesHandler : IMessageHandler
    {
        ...
        private async Task<HandleMessageResult> HandleRenegotiation(Shared.Models.Requests.WebRTCNegotiation request, BaseMessage message)
        {
            ...
            _logger.LogWarning("ReactOnOfferRequest event error. Requested room not exist.");
        ...
        private HandleMessageResult HandleOffer(Shared.Models.Requests.WebRTCNegotiation request, BaseMessage message)
        {
            ...
            if (_coordinator.GetVideoSession(request.RoomId, out var session) == SessionRequestResult.NoError)
            {
                ...
                _logger.LogWarning("ReactOnOffer event error. Requested peer not exist.");
                return HandleMessageResult.InternalError;
            }

            _logger.LogWarning("ReactOnOffer event error. Requested room not exist.");
            return HandleMessageResult.InternalError;
        }
        ...
        private async Task<HandleMessageResult> HandleOfferRequest(Shared.Models.Requests.WebRTCNegotiation request, BaseMessage message)
        {
            ...
            if (_coordinator.GetVideoSession(request.RoomId, out var session) == SessionRequestResult.NoError)
            {
                ...
                _logger.LogWarning("ReactOnOfferRequest event error. Requested peer not exist.");
                SendResponse(request, WebRTCNegotiationResult.InternalError, message);
                return HandleMessageResult.InternalError;
            }
            ...
            if (_coordinator.GetVideoSession(request.RoomId, out var session) == SessionRequestResult.NoError)
            {
                if (session!.GetPeerById(request.PeerId, out _))
                {
                    if (sdpAnswer != null)
                    {
                        ...
                        _logger.LogInformation("ReactOnAnswer event completed.");
                        return HandleMessageResult.NoError;
                    }
                    
                    _logger.LogWarning("ReactOnAnswer event error. Null SDP.");
                    return HandleMessageResult.InternalError;
                }
                
                _logger.LogWarning("ReactOnAnswer event error. Requested peer not exist.");
                return HandleMessageResult.InternalError;
            }

            _logger.LogWarning("ReactOnAnswer event error. Requested room not exist.");
            return HandleMessageResult.InternalError;
        }
        ...
        private HandleMessageResult HandleICE(Shared.Models.Requests.WebRTCNegotiation request, BaseMessage message)
        {
            ...
            if (_coordinator.GetVideoSession(request.RoomId, out var session) == SessionRequestResult.NoError)
            {
                if (session.GetPeerById(request.PeerId, out _))
                {
                    ...
                    _logger.LogDebug("ReactOnICE event completed.");
                    return HandleMessageResult.NoError;
                }

                _logger.LogWarning("ReactOnICE event error. Requested peer not exist.");
                return HandleMessageResult.InternalError;
            }

            _logger.LogWarning("ReactOnICE event error. Requested room not exist.");
            return HandleMessageResult.InternalError;
        }
